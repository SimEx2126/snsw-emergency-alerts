...
export class MemoryManager {
  constructor(runsDir, options = {}) {
    this.runsDir = runsDir;
    // Threshold overrides from crucix.config.mjs (delta.thresholds)
  ...
  addRun(synthesizedData) {
    ...
    const delta = computeDelta(synthesizedData, previous, this.thresholds, priorRuns);
  ...
  // Hazard incident identities (guid/id) MUST survive compaction: the delta
  // engine dedups new-incident signals against these hot runs.
  _compactForStorage(data) {
    return {
      meta: data.meta,
      rfs: {
        total: data.rfs?.total || 0,
        emergency: (data.rfs?.emergency || []).length,
        watchAct: (data.rfs?.watchAct || []).length,
        incidents: (data.rfs?.incidents || []).map(i => ({
          guid: i.guid, category: i.category, title: i.title,
        })),
      },
      bom: {
        total: data.bom?.total || 0,
        summary: data.bom?.summary || {},
        warnings: (data.bom?.warnings || []).map(w => ({
          id: w.id, type: w.type, floodClass: w.floodClass, title: w.title,
        })),
      },
      quakes: {
        total: data.quakes?.total || 0,
        maxMag: data.quakes?.maxMag ?? null,
        events: (data.quakes?.events || []).map(e => ({ id: e.id, mag: e.mag })),
      },
      fred: data.fred,
      energy: data.energy,
      bls: data.bls,
