...
export function evaluateHazardSignals(signals, delta) {
  const outages = signals.filter(s => (s.key || '').startsWith('source_down:'));
  ...
  const incidentSignals = signals.filter(s => isHazardSignal(s) && s.item && !s.sourceOutage);
  ...
  // ── PRIORITY: a data source has gone dark. Ranked below live hazards but
  //    above advisories — a monitor that cannot see is a safety problem.
  if (outages.length > 0) {
    const names = outages.map(o => o.sourceName || 'a data source');
    return {
      shouldAlert: true, tier: 'PRIORITY', confidence: 'HIGH',
      headline: `Data source unavailable — ${names.join(', ')}`,
      reason: `The monitor has failed to reach ${names.join(' and ')} for several consecutive sweeps. Warnings from ${names.length > 1 ? 'these sources' : 'this source'} may be missing from the dashboard and from these alerts.`,
      actionable: 'Check the official RFS, SES and BOM sites directly until this clears, and let whoever maintains the monitor know.',
      signals: outages.map(o => o.reason || o.key),
      crossCorrelation: 'system health',
    };
  }
